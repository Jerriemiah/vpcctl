...
def setup_internet_gateway(vpc, ns_name, host_iface="enp0s3"):
    ...
    print(f"🌍 Setting up Internet Gateway for {vpc} ({veth_host} ↔ {veth_ns}); link {link_cidr}")
    ...
    print(f"✅ Internet Gateway configured for {vpc} (vpc_cidr={vpc_cidr}).")
...
def create_vpc(args):
    ...
    ensure_state_dir()
    if load_state(vpc):
        print(f"⚠️  VPC {vpc} already exists (idempotent).")
        return

    ns = vpc_ns(vpc)
    br = bridge_name(vpc)

    print(f"🧱 Creating VPC {vpc} (CIDR={cidr}) with namespace {ns} and bridge {br}")
...
def add_subnet(args):
    ...
    if not state:
        sys.exit(f"❌ VPC {vpc} not found. Create it first.")

    if subnet in state["subnets"]:
        print(f"⚠️ Subnet {subnet} already exists (idempotent).")
        return
    ...
    print(f"🧱 Creating subnet {subnet} ({cidr}) inside VPC {vpc}...")
    ...
    run(["ip", "link", "add", veth_host, "type", "veth", "peer", "name", veth_ns])
    # Move host side into VPC namespace
    run(["ip", "link", "set", veth_host, "netns", vpc_ns_name])
    # Move ns side into subnet namespace
    run(["ip", "link", "set", veth_ns, "netns", ns_subnet])
    ...
    # Inside subnet namespace: bring up interfaces and loopback
    run(["ip", "netns", "exec", ns_subnet, "ip", "link", "set", veth_ns, "up"])
    run(["ip", "netns", "exec", ns_subnet, "ip", "link", "set", "lo", "up"])


    # Compute gateway for this subnet: use .1 in the subnet
    # Accept subnets like 10.10.1.0/24, 192.168.5.0/24
    try:
        net_base = cidr.split('/')[0]
        # base without last octet
    ...
    # Assign IP inside subnet namespace
    run(["ip", "netns", "exec", ns_subnet, "ip", "addr", "add", f"{ns_ip}/{mask}", "dev", veth_ns])

    # Add default route inside namespace via gateway (which must be on the bridge)
    run(["ip", "netns", "exec", ns_subnet, "ip", "route", "add", "default", "via", gw_ip])

    # Add gateway IP to the bridge inside the VPC namespace.
    # Idempotent: attempt once, allow non-zero return (address might already exist).
    ...
    save_state(state)
    print(f"✅ Subnet {subnet} added to {vpc} (ns={ns_subnet}, gw={gw_ip})")

    # Setup NAT if public
    if is_public:
        print(f"🌍 Enabling NAT for {subnet} via {INTERNET_IF}...")
        # Enable forwarding in the VPC namespace
        run(["ip", "netns", "exec", vpc_ns_name, "sysctl", "-w", "net.ipv4.ip_forward=1"])
        # NAT rule: -s must match the subnet CIDR
        run(["ip", "netns", "exec", vpc_ns_name, "iptables", "-t", "nat", "-A", "POSTROUTING",
             "-s", cidr, "-o", INTERNET_IF, "-j", "MASQUERADE"])
        run(["ip", "netns", "exec", vpc_ns_name, "iptables", "-A", "FORWARD", "-i", br, "-o", INTERNET_IF, "-j", "ACCEPT"])
        run(["ip", "netns", "exec", vpc_ns_name, "iptables", "-A", "FORWARD", "-o", br, "-m", "state",
...
def add_subnet(args):
    ...
    # 🟩 NEW SECTION: ensure bridge itself is UP and forwards packets inside VPC ns
    ...
    # 🟩 NEW SECTION: bring interfaces up inside the subnet namespace
    run(["ip", "netns", "exec", ns_subnet, "ip", "link", "set", veth_ns, "up"])
    run(["ip", "netns", "exec", ns_subnet, "ip", "link", "set", "lo", "up"])
...
def peer_vpcs(args):
    ...
    if not state_a or not state_b:
        sys.exit("❌ One or both VPCs not found. Create them first.")
    ...
    print(f"🔗 Creating peering veth pair {if_a} <-> {if_b} between {a} and {b}")
    ...
    print(f"📡 Adding static routes: {cidr_a} ↔ {cidr_b}")
    ...
    print(f"✅ Peering established between {a} and {b}. Cross-VPC routes installed.")
